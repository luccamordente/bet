...
function reportToString(report: Report): string {
  ...
  for (const {house, sports} of report) {
    ...
    for (const sport of sports) {
      text += `\n  ${getStatusIcon(sport.status)} ${sportToString(sport.name)}: ${sport.count}`
    }
    text += `\n`;
  }
  return text;
...
function mergeWithBaseReport(report: Report): Report {
  ...
  base.forEach(baseItem => {
    const reportItem = report.find(item => item.house.name === baseItem.house.name);
    if (reportItem === undefined) {
      return;
    }

    baseItem.sports.forEach((baseItemSport, j) => {
      const reportItemSport = reportItem.sports.find(sport => sport.name === baseItemSport.name);
