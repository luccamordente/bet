...
import {
  save as saveBettable,
  Bettable,
  BettableMarket,
  MarketKey,
} from "../models/bettable";
...
async function* getMatchBets(matchId: Id) {
  ...
  let extractTime;
  try {
    markets = (await axios.get(`/matchups/${matchId}/markets/related/straight`))
      .data;
...
async function getLeagueMatches(leagueId: Id): Promise<Match[]> {
  ...
  try {
    ...
    console.error(
      `Error getting league matches from league ${leagueId}`,
      error,
    );
    return [];
  }
...
function filterMatches(matches: Match[]): Match[] {
  return matches.filter((match: Match) => {
    return (
      moment(match.startTime).isBetween(
        moment.now(),
        moment().add(TIME_SPAN_HOURS, "hours"),
      ) &&
      match.type === "matchup" &&
      match.units === "Regular"
    );
...
function filterMarkets(
  markets: Market[],
  extraFilter: (market: Market) => boolean,
): Market[] {
  return markets.filter((market) => {
    return (
      ALLOWED_MARKETS.some((allowed) => {
